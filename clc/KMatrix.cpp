...
double& KMatrix::operator()(int r, int c){
    if (r >= mtrx.rows() || c >= mtrx.cols()) return null_double;
...
//        cout << "ERROR: Failed to create Eigen::Vector3d from KMatrix." << endl;
		KMatrix km(1, 1);
		km(1, 1) = -1;
		return km;
	}
...
/*
Transpose Matrix. Does not modify origional matrix, rather returns a new translated matrix.
...
/*
Return conjugate of matrix. Does not modify origional matrix.
...
/*
Calculate conjugate transpose matrix. Does not modify origional matrix.
...
/*
Calculate inverse matrix. Does not modify origional matrix.
...
void KMatrix::print(int indent, bool spaces){
	...
	for (int r = 0; r < KMatrix::mtrx.rows() ; r++){
		...
		cout << KMatrix::mtrx(r, KMatrix::mtrx.cols()-1) << " |";
	}
